...
import { DepositsData } from "@/lib/data";
import { useEffect, useState } from "react";
import { IoIosSearch } from "react-icons/io";
import { useSearchParams } from "react-router-dom";

export default function PendingDesposits() {
  const [searchTerm, setSearchTerm] = useState("");
  const [searchedData, setSearchedData] = useState(DepositsData);
  ...
  function handleSearchedData(currentSearch) {
    ...
    const searched = DepositsData.tableRows.filter((row) => {
  ...
  return (
    <div className="space-y-10 px-6 py-12">
      <div className="flex w-full flex-col gap-5 md:flex-row md:items-center md:justify-between md:gap-0">
        <h3 className="text-xl font-bold text-gray-700">Pending Deposits</h3>
